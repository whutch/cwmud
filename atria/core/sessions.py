...
class _Session(HasFlags):
    ...
    def _check_idle(self):
        ...
        if idle >= settings.IDLE_TIME:
            if idle >= settings.IDLE_TIME_MAX:
                # They've been idle long enough, dump them.
    ...
    def _parse_input(self, data):
        ...
        if data.strip() == "quit":
            self.close("Okay, goodbye!", log_msg=joins(self, "has quit"))
        else:
            self.send("You sent:", data)
    ...
    def poll(self):
        """Check status and process any queued IO."""
