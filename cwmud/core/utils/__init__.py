...
def generate_hash(string):
    ...
    :returns str: The generated hash
...
def check_hash(string, hashed_string):
    ...
    :returns bool: Whether the input string matches the given hash
...
def recursive_import(base_path, base_package):
    """Recursively import a package and all submodules or subpackages.

    :param str base_path: Path to the top level package
    :param str base_package: Import path to top level package
    :returns None:

    """
    def _import_package(path, prefix):
        import_module(prefix, base_package)
        for finder, name, is_pkg in iter_modules([path]):
            if is_pkg:
                _import_package(join(path, name), ".".join((prefix, name)))
            else:
                import_module(".".join((prefix, name)), base_package)
    _import_package(base_path, base_package)
